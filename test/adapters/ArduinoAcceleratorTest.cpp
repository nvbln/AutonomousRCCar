...
TEST(ArduinoAcceleratorTests, callbacksOnlyAfter10ms) {
  ...
  auto mockIMUAccelerator = NiceMock<MockIMUAccelerator>();
  auto mockClock = MockClock();
  ...
  auto arduinoAccelerator = ArduinoAccelerator(&mockSerial, &mockIMUAccelerator, &mockClock);

  bool calledBack = false;

  arduinoAccelerator.addCallback([&calledBack](AccelerationData _) { calledBack = true; });
